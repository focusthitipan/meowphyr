...
export default function AppDetailsPage() {
  ...
  const changeLocationMutation = useMutation({
    ...
    onSuccess: async () => {
      await invalidateAppQuery(queryClient, { appId });
      await refreshApps();
      showSuccess(t("appDetails.appLocationUpdated"));
    },
    onError: (error) => {
      showError(error);
    },
    ...
    return (
      <div className="relative min-h-screen p-8">
        ...
        <div className="flex flex-col items-center justify-center h-full">
          <h2 className="text-xl font-bold">{t("appDetails.appNotFound")}</h2>
        </div>
      </div>
    );
  }
      ...
      <div className="w-full max-w-2xl mx-auto mt-10 p-4 bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 shadow-sm relative">
        <div className="flex items-center mb-3">
          ...
          <Tooltip>
            ...
            <TooltipContent>
              {selectedApp.isFavorite
                ? t("appDetails.removeFromFavorites")
                : t("appDetails.addToFavorites")}
            </TooltipContent>
          </Tooltip>
          <Button
            variant="ghost"
        ...
        <Dialog open={isRenameDialogOpen} onOpenChange={setIsRenameDialogOpen}>
          <DialogContent className="max-w-sm p-4">
            ...
            <Input
              ...
              placeholder={t("appDetails.enterNewAppName")}
              className="my-2"
              autoFocus
            />
        ...
        >
          <DialogContent className="max-w-sm p-4">
            <DialogHeader className="pb-2">
              <DialogTitle>{t("appDetails.renameAppFolder")}</DialogTitle>
              <DialogDescription className="text-xs">
                {t("appDetails.renameAppFolderDesc")}
              </DialogDescription>
            </DialogHeader>
            <Input
              value={newFolderName}
              onChange={(e) => setNewFolderName(e.target.value)}
              placeholder={t("appDetails.enterNewFolderName")}
              className="my-2"
              autoFocus
            />
            <DialogFooter className="pt-2">
              ...
              >
                {isRenamingFolder ? (
                  <>
                    ...
                    {t("appDetails.renaming")}
                  </>
                ) : (
                  t("appDetails.renameFolderBtn")
                )}
              </Button>
            </DialogFooter>
          </DialogContent>
          ...
          <DialogContent className="max-w-sm p-4">
            <DialogHeader className="pb-2">
              <DialogTitle className="text-base">
                {t("appDetails.howToRename", { name: selectedApp.name })}
              </DialogTitle>
              <DialogDescription className="text-xs">
                {t("appDetails.chooseOption")}
              </DialogDescription>
            </DialogHeader>
            <div className="space-y-2 my-2">
              ...
              >
                <div className="absolute top-1 right-1">
                  <span className="bg-blue-100 text-blue-800 text-xs font-medium px-1.5 py-0.5 rounded dark:bg-blue-900 dark:text-blue-300 text-[10px]">
                    {t("appDetails.recommended")}
                  </span>
                </div>
                <div className="text-left">
                  <p className="font-medium text-xs">{t("appDetails.renameAppAndFolder")}</p>
                  <p className="text-xs text-gray-500 dark:text-gray-400">
                    {t("appDetails.renameAppAndFolderDesc")}
                  </p>
                </div>
              </Button>
              ...
              >
                <div className="text-left">
                  <p className="font-medium text-xs">{t("appDetails.renameAppOnly")}</p>
                  <p className="text-xs text-gray-500 dark:text-gray-400">
                    {t("appDetails.renameAppOnlyDesc")}
                  </p>
                </div>
              </Button>
            </div>
        ...
        {selectedApp && (
          <Dialog open={isCopyDialogOpen} onOpenChange={setIsCopyDialogOpen}>
            <DialogContent className="max-w-md p-4">
              <DialogHeader className="pb-2">
                <DialogTitle>{t("appDetails.copyTitle", { name: selectedApp.name })}</DialogTitle>
                <DialogDescription className="text-sm">
                  <p>{t("appDetails.copyDescription")}</p>
                  <p>{t("appDetails.copyNote")}</p>
                </DialogDescription>
              </DialogHeader>
              <div className="space-y-3 my-2">
                <div>
                  <Label htmlFor="newAppName">{t("appDetails.newAppName")}</Label>
                  ...
                  {nameExists && (
                    <p className="text-xs text-yellow-600 dark:text-yellow-500 mt-1">
                      {t("appDetails.nameAlreadyExists")}
                    </p>
                  )}
                </div>
                ...
                <div className="space-y-2">
                  ...
                  >
                    ...
                    <div className="text-left">
                      ...
                      <p className="text-xs text-gray-500 dark:text-gray-400">
                        {t("appDetails.copyWithHistoryDesc")}
                      </p>
                    </div>
                  </Button>
                  ...
                  >
                    ...
                    <div className="text-left">
                      ...
                      <p className="text-xs text-gray-500 dark:text-gray-400">
                        {t("appDetails.copyWithoutHistoryDesc")}
                      </p>
                    </div>
                  </Button>
                </div>
        ...
        >
          <DialogContent className="max-w-sm p-4">
            <DialogHeader className="pb-2">
              ...
              <DialogDescription className="text-xs">
                {t("appDetails.changeLocationDesc")}
              </DialogDescription>
            </DialogHeader>
            <DialogFooter className="pt-2">
              <Button
        ...
        <Dialog open={isDeleteDialogOpen} onOpenChange={setIsDeleteDialogOpen}>
          <DialogContent className="max-w-sm p-4">
            <DialogHeader className="pb-2">
              <DialogTitle>{t("appDetails.deleteTitle", { name: selectedApp.name })}</DialogTitle>
              <DialogDescription className="text-xs">
