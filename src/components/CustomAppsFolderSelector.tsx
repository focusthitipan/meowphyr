...
export function CustomAppsFolderSelector() {
  ...
  const handleResetToDefault = async () => {
    try {
      ...
      await fetchCustomAppsFolder();
      showSuccess("Meowphyr apps folder reset successfully");
    } catch (error: any) {
      showError(`Failed to reset Meowphyr Apps folder path: ${error.message}`);
    }
  };
  ...
  const fetchCustomAppsFolder = async () => {
    try {
      ...
      showError(`Failed to fetch Meowphyr apps folder path: ${error.message}`);
    }
  };
    ...
    <div className="space-y-4">
      <div className="space-y-2">
        ...
        <div className="text-sm text-gray-500 dark:text-gray-400">
          <p>
            {isPathAvailable
              ? "This is the top-level folder that Meowphyr will store new applications in."
